...
[Serializable] public class ShockWaveInstancer
{
    [field: SerializeField] public Instancer[] targetObj;
    [SerializeField] private GameObject parent;
    [SerializeField] private List<ShockWaveParameter> parameter = new List<ShockWaveParameter>();
    ...
    {
        targetObj[0].Obj = obj;
        foreach(Instancer ui in targetObj)
        {
            ui.Initialize();
        }
    }
    ...
    {
        if (instanceSound != null) { World.instance.audioSource.PlayOneShot(instanceSound); }

        for (int i = 0; i < targetObj.Length; i++)
        {
            targetObj[i].Instance(parent);
            GameObject clone = targetObj[i].Last;
            clScript = clone.gameObject.AddComponent<ShockWave>();
            clScript.Initialize(clone, parameter[i]);
        }
    }
    ...
    public bool Displaying
    {
        get
        {
            foreach(Instancer ui in targetObj)
            {
                if(ui.state != Instancer.DisplayState.Death) { return true; }
            }
